...
const debug = require("debug")("lib:redisPool");
...
function RedisPool (name, redisOptions, poolOptions) {
  ...
  let _pool = null;
  const _redisOptions = redisOptions;
  const _poolOptions = Object.assign(poolOptions, {name: name});

  // Build new Redis database clients.
  _poolOptions.create = function (cb) {
    const client =  redis.createClient(_redisOptions);
